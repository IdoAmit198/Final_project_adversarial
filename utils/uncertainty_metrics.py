...
def gamma_correlation(samples_certainties, sort=True):
    ...
    if (n_s + n_d) == 0:
        smoothing = 0.000001
    results['gamma'] = (n_s - n_d) / (n_s + n_d + smoothing)
    results['AUROC'] = (n_s) / (n_s + n_d + smoothing)
    results['n_s'] = n_s
    results['n_d'] = n_d
    return results
...
def coverage_for_desired_accuracy(samples_certainties, accuracy=0.95, sort=False, start_index=0):
    ...
    if coverage_for_accuracy==0 and cummean_correctness[-1]>accuracy:
        coverage_for_accuracy = num_samples
        # exit()
    # print(f"coverage_for_accuracy: {coverage_for_accuracy}")
...
def log_uncertainty(samples_certainties, rc_curve_save_pth:str):
    uncertainty_dict = {}
    ...
    ranking = gamma_correlation(samples_certainties)
    uncertainty_dict['AUROC'] = ranking['AUROC']
    uncertainty_dict['AURC'] = AURC_calc(samples_certainties)
    ece15, _ = ECE_calc(samples_certainties, num_bins=15)
    uncertainty_dict['ece15'] = ece15.item()
    adaptive_ece , _ = ECE_calc(samples_certainties, num_bins=15,
                                                              bin_boundaries_scheme=calc_adaptive_bin_size)
    uncertainty_dict['adaptive_ece'] = adaptive_ece.item()
    uncertainty_dict['confidence_mean'] = confidence_mean(samples_certainties)

    uncertainty_dict['coverage_accuracy_95'] = coverage_for_desired_accuracy(samples_certainties, accuracy=0.95,
                                                         start_index=200, sort=True)
    uncertainty_dict['coverage_accuracy_90'] = coverage_for_desired_accuracy(samples_certainties, accuracy=0.90,
                                                         start_index=200, sort=True)
    uncertainty_dict['coverage_accuracy_80'] = coverage_for_desired_accuracy(samples_certainties, accuracy=0.80,
                                                         start_index=200, sort=True)
    uncertainty_dict['coverage_accuracy_70'] = coverage_for_desired_accuracy(samples_certainties, accuracy=0.70,
                                                         start_index=200, sort=True)
    uncertainty_dict['coverage_accuracy_50'] = coverage_for_desired_accuracy(samples_certainties, accuracy=0.50,
                                                                             start_index=200, sort=True)

    selective_risks, coverages = selective_risks_calc(sorted_samples_certainties, sort=False)
    best_selective_risks, _ = best_selective_risks_calc(sorted_samples_certainties)
    rc_curve_resolution = 1000
    if selective_risks.shape[0] < 10000:
        rc_curve_resolution = 10
    selected_indices_to_present = torch.arange(1, selective_risks.shape[0] + 1) % (selective_risks.shape[0] / rc_curve_resolution) == 0
    try:  # If resolution is too low, rc curve is meaningless
        # wandb.log({f'rc_curves/rc_curve':
        ...
        #         keys=['softmax', 'best'],
        #         title=f'rc_curves/rc_curve',
        #         xname='coverage')}, step=epoch)
        # Transfer the above wandb code to use matplotlib.pyplot instead
        # rc_curve_tuple = namedtuple('rc_curve', ['coverages', 'selective_risks', 'best_selective_risks'])
        rc_curve_tuple = (coverages[selected_indices_to_present].tolist(),
                                        selective_risks[selected_indices_to_present].tolist(),
                                        best_selective_risks[selected_indices_to_present].tolist())
        with open(rc_curve_save_pth, 'wb') as f:
            pickle.dump(rc_curve_tuple, f)
